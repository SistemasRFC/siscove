...
include_once("../../Dao/Graficos/GraficosVendaDao.php");
    ...
    Public Function SelecionaDados($Json=true){
        ...
        if ($lista[0]){
            for($i=0;$i<count($lista[1]);$i++){
                $lista[1][$i]['VLR_VENDA_FECHADA'] = number_format($lista[1][$i]['VLR_VENDA_FECHADA'],2,".","");
                $lista[1][$i]['VLR_VENDA_ABERTA'] = number_format($lista[1][$i]['VLR_VENDA_ABERTA'],2,".","");
                $lista[1][$i]['VLR_VENDA_CANCELADA'] = number_format($lista[1][$i]['VLR_VENDA_CANCELADA'],2,".","");
