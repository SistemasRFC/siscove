...
$(function() {
    ...
    $("#btnCaixasFechados").click(function(){
        CarregaListaCaixasFechadosGerencia();
    });   
    $("#btnPesquisar").click(function(){
        CarregaCaixasVendedor(); 
    });
});

function CarregaCaixasVendedor(){
    $("#tabelaPagamentos").html('');
    $( "#dialogInformacao" ).jqxWindow('setContent', "Aguarde!");
    $( "#dialogInformacao" ).jqxWindow("open");
    $.post('../../Controller/CaixaGerencia/CaixaGerenciaController.php',{
        method: 'ListarCaixasVendedor',
...
function MontaTabelaPagamentosGerencia(pagamentosVendedor, pagamentosTipo){
    ...
    impressao += '<tr><td align="center"><span style="font-size: 20;">Fechamento de caixa da Ger&ecirc;ncia</span></td></tr>';
    ...
    tabela += '<tr>';
    tabela += '<td class="TDTitulo">Vendedor</td>';
    tabela += '<td class="TDTitulo">Data</td>';
    ...
    for(i=0; i<pagamentosVendedor.length; i++){
        ...
        tabela += '<td>'+pagamentosVendedor[i].NME_USUARIO_COMPLETO+'</td>';
        tabela += '<td>'+pagamentosVendedor[i].DTA_CAIXA+'</td>';
...
function FecharCaixa(){
    ...
    $( "#dialogInformacao" ).jqxWindow("open");
    $.post('../../Controller/CaixaGerencia/CaixaGerenciaController.php',{
        method: 'FecharCaixaGerencia',
        dtaPagamento: $("#dtaPagamento").val()
    },function(data){
        data = eval('('+data+')');
        if (data[0]){
            impressao = impressao.replace("#data#", data[1][0].DTA_CAIXA_GERENCIA);          
            PreparaImpresao();      
