...
class GraficosVendaDao extends BaseDao
{
    ...
    Public Function SelecionaDados($codClienteFinal){
        $sql_lista = "SELECT MONTH(V.DTA_VENDA) AS MES, 
                             SUM(CASE WHEN V.NRO_STATUS_VENDA = 'F' THEN VP.VLR_VENDA ELSE 0 END) AS VLR_VENDA_FECHADA,
                             SUM(CASE WHEN V.NRO_STATUS_VENDA = 'A' THEN VP.VLR_VENDA ELSE 0 END) AS VLR_VENDA_ABERTA,
                             SUM(CASE WHEN V.NRO_STATUS_VENDA = 'C' THEN VP.VLR_VENDA ELSE 0 END) AS VLR_VENDA_CANCELADA,
