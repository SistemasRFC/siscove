...
class VendaReferenciaDao extends BaseDao
{
    Protected $tableName = "EN_VENDA_REFERENCIA";
    ...
    Public Function VendaReferenciaDao(){
        $this->conect();
    }
    ...
    Public Function UpdateVendaReferencia($nroSequencial, $indStatus, $codUsuario){
        $sql = "UPDATE EN_VENDA_REFERENCIA 
    ...
    Public Function InsertVendaReferencia($codUsuario){
        $nroSequencial = $this->CatchUltimoCodigo('EN_VENDA_REFERENCIA', 'NRO_SEQUENCIAL');
        $sql = "INSERT INTO EN_VENDA_REFERENCIA(NRO_SEQUENCIAL, COD_VENDA, IND_STATUS_REFERENCIA, DTA_EMISSAO_NOTA, COD_USUARIO)
    ...
    Public Function RetornaVendaReferencia(){
        $sql = "SELECT COALESCE(NRO_SEQUENCIAL,0) AS NRO_SEQUENCIAL,
                       COD_VENDA,
                       DTA_EMISSAO_NOTA,
                       IND_STATUS_REFERENCIA
                  FROM EN_VENDA_REFERENCIA 
                 WHERE COD_VENDA = ".$this->Populate('codVenda', 'I')."
                   AND IND_STATUS_REFERENCIA IN ('A', 'E')
                   AND NRO_SEQUENCIAL = (SELECT MAX(NRO_SEQUENCIAL) FROM EN_VENDA_REFERENCIA WHERE COD_VENDA = ".$this->Populate('codVenda', 'I').")";
...
//        echo $sql; die;
    ...
    Public Function RemoveReferencia($nroSequencial){
        $sql = "DELETE FROM EN_VENDA_REFERENCIA WHERE NRO_SEQUENCIAL = ".$nroSequencial;
        return $this->insertDB($sql);
    }
}